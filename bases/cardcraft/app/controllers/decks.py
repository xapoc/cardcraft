...
async def list_decks():
    ...
    decks = await gamedb.decks.find({"owner": sess_id}).to_list()
    listing: list = [
        "p",
        {"class": "blank"},
        [
    ...
    return hiccpage([trident(menu(), listing, ["p", "No deck selected"])])
...
async def show_deck(deck_id: str):
    ...
    deck: dict = await gamedb.decks.find_one({"id": deck_id, "owner": sess_id})
...
async def store_deck():
    ...
    await gamedb.decks.insert_one(
        {"id": os.urandom(16).hex(), "name": name, "cards": cards, "owner": sess_id}
    )
...
async def update_deck(deck_id: str):
    ...
    await gamedb.decks.update_one(
        {"id": deck_id}, {"$set": {"name": name, "cards": cards, "owner": sess_id}}
    )
